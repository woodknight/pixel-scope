...
namespace pixelscope::ui
{
  ...
  {
#if defined(_WIN32)
    // Declare per-monitor DPI awareness before SDL_Init so Windows does not
    // scale the window for us.  Without this, a DPI-unaware process on a
    // high-DPI display (e.g. 150 %) has its entire output stretched to fill
    // the screen, making the window appear fullscreen and blurring all text.
    SDL_SetHint(SDL_HINT_WINDOWS_DPI_AWARENESS, "permonitorv2");
#endif
    // SDL2 derives the Linux desktop shell identity from these environment
    // variables before SDL_Init. Keep them in sync with pixelscope.desktop so
    // docks can associate the window with the launcher and icon.
    SDL_setenv("SDL_VIDEO_X11_WMCLASS", "pixelscope", 1);
    SDL_setenv("SDL_VIDEO_WAYLAND_WMCLASS", "pixelscope", 1);
    SDL_SetHint(SDL_HINT_APP_NAME, "PixelScope");
    ...
    // Clamp the initial window size to 90 % of the usable display area so that
    // the window never extends off-screen on any display size or DPI setting.
    int w = kWindowWidth;
    int h = kWindowHeight;
    SDL_Rect usable{};
    ...
    window_ = SDL_CreateWindow("PixelScope", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h,

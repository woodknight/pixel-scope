...
namespace pixelscope::platform
{
  ...
  std::filesystem::path resolve_icon_path()
  {
    static const char *kIconFilename = "icon_256x256.png";

    ScopedSdlString base_path(SDL_GetBasePath());
    if (base_path.get() != nullptr)
    {
      // Installed layout: <prefix>/bin/../share/pixelscope/icons/
      std::error_code ec;
      const auto installed =
          std::filesystem::path(base_path.get()) / ".." / "share" / "pixelscope" /
          "icons" / kIconFilename;
      if (std::filesystem::exists(installed, ec) && !ec)
      {
        return installed;
      }
    }
